...
///
/// A function is the combination of a reference to an oxidd::manager and a
/// (possibly tagged) edge pointing to a node.
///
/// ### Reference Counting
///
...
/// modelling the oxidd::function concept.
///
/// ### Reference Counting
///
...
/// - `f.pick_cube()` returns a satisfying oxidd::assignment if `f` is
///   satisfiable, or an empty oxidd::assignment otherwise. `f` must be valid.
///
...
/// Quantification extension for oxidd::boolean_function
///
...
/// - `m.top()` returns the constant true Boolean function ⊤. `m` must be valid.
///   Acquires a shared manager lock.
/// - `m.bot()` returns the constant false Boolean function ⊥. `m` must be
///   valid. Acquires a shared manager lock.
///
...
concept boolean_function_manager =
    ...
    requires(const M m, M mut_m) {
      ...
      { m.top() } -> std::same_as<typename M::function>;
      { m.bot() } -> std::same_as<typename M::function>;
    };
