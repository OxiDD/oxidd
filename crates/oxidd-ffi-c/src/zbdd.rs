...
/// `oxidd_zbdd_manager_var_name()` works fine. In other cases, one can use this
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn oxidd_zbdd_manager_with_var_name(
    manager: zbdd_manager_t,
    var: VarNo,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn oxidd_zbdd_sat_count(f: zbdd_t, vars: LevelNo) -> util::num::natural_t {
    let f = unsafe { f.get() }.expect(FUNC_UNWRAP_MSG);
