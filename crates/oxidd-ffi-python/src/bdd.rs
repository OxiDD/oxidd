...
impl BDDManager {
    ...
    fn import_dddmp<'py>(
        ...
        PyList::new(py, imported.into_iter().map(BDDFunction))
    }
    ...
    fn export_dddmp<'py>(
        ...
        crate::util::export_dddmp::<BDDFunction>(
            &self.0,
            &path,
            functions,
            version,
    ...
    fn export_dddmp_with_names<'py>(
        ...
        crate::util::export_dddmp_with_names::<BDDFunction>(
            &self.0,
            &path,
            functions,
            version,
    ...
    fn visualize<'py>(
        ...
        let mut iter = crate::util::TryIter::<BDDFunction>::try_from(functions)?;
        visualizer = self
            .0
    ...
    fn visualize_with_names<'py>(
        ...
        let mut visualizer = oxidd_dump::Visualizer::new().port(port);
        let mut iter = crate::util::FuncStrPairIter::<BDDFunction>::try_from(functions)?;
    ...
    fn dump_all_dot<'py>(
        ...
        crate::util::dump_all_dot::<BDDFunction>(&self.0, &path, functions)
    }
...
impl BDDSubstitution {
    ...
    fn new(pairs: &Bound<PyAny>) -> PyResult<Self> {
        ...
        for pair in pairs.try_iter()? {
            ...
            vars.push(v.extract()?);
            replacements.push(r.cast::<BDDFunction>()?.get().0.clone());
        }
...
impl std::ops::Deref for BDDFunction {
    type Target = oxidd::bdd::BDDFunction;
